...
Because the database is very large, usually for game rules, we need to build an 80GB database, which can only be used on PC or put the database on the server and query through App. Because the database is huge, it is unrealistic to build a database of all rule variants, so this kind of program usually only supports Nine Men's Morris of standard rules.
...
1) The piece to be moved is removed from the board, the key  is  `psq(type1) x1`, ("represents a bit difference or operation, the same as below"; 
